...
extension DKMDumper {
    ...
    internal func format(_ command: DKMCommand) -> String { // swiftlint:disable:this function_body_length
        ...
        case let .filterLine(line):
            ...
            result += spacer() + format(line.startPitch)
            result += spacer() + format(line.endPitch)
            result += spacer() + format(line.startBandwidth)
            result += spacer() + format(line.endBandwidth)
        ...
        case let .tempoLine(line):
            ...
            result += spacer() + format(line.startBeat)
            result += spacer() + format(line.duration)
            result += spacer() + format(line.startTempo)
            result += spacer() + format(line.endTempo)
