...
extension IvorDumper {
    ...
    private static let accidentals: [MXLPitch.Accidental: String] = [.doubleFlat: "𝄫",
                                                                     .doubleSharp: "𝄪",
                                                                     .flat: "♭",
                                                                     .natural: "♮",
                                                                     .sharp: "♯"]

    private static let letters: [MXLPitch.Letter: String] = [.a: "A",
                                                             .b: "B",
                                                             .c: "C",
                                                             .d: "D",
                                                             .e: "E",
    ...
    private func _format(_ graceNote: MXLGraceNote) -> String {
        ...
        if let result = _format(graceNote.ties) {
            line += spacer()
            line += result
        }
    ...
    private func _format(_ note: MXLNote) -> String {
        ...
        if let result = _format(note.ties) {
            line += spacer()
            line += result
        }
    ...
    private func _format(_ pitch: MXLPitch) -> String {
        guard let letter = Self.letters[pitch.letter],
              let accidental = Self.accidentals[pitch.accidental]
        else { return "\(pitch)" }
    ...
    private func _format(_ ties: Set<MXLTie>) -> String? {
        let hasStart = ties.contains(.start)
        let hasStop = ties.contains(.stop)
