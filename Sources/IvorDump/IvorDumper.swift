...
extension IvorDumper {
    ...
    ///
    /// - Returns:  `true` if the file was dumped successfully; otherwise, `false`. An unrecognized file format, or
    ///             a recognized parsing error, is written to standard error rather than thrown, so that a multi-file
    ///             run can continue past a bad file.
    /// - Throws:   ``IvorDumper/Error/dumpFailure(_:)`` if any other error occurs while dumping the file.
    @discardableResult
